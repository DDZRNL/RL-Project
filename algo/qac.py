...
@register_algo_factory_func("qac")
...
class QAC(PolicyAlgo, ValueAlgo):
    ...
    def __init__(self, **kwargs):
        ...
        self.automatic_qac_tuning = kwargs["algo_config"].critic.target_q_gap is not None and \
        ...
        self.target_q_gap = self.algo_config.critic.target_q_gap if self.automatic_qac_tuning else 0.0
    ...
    @property
    def log_qac_weight(self):
        return self.nets["log_qac_weight"]() if self.automatic_qac_tuning else\
    ...
    def _create_networks(self):
        ...
        # QAC (if automatically tuning)
        if self.automatic_qac_tuning:
            self.nets["log_qac_weight"] = BaseNets.Parameter(torch.zeros(1))
    ...
    def _create_optimizers(self):
        ...
        # qac (if automatically tuning)
        if self.automatic_qac_tuning:
            self.optimizers["qac"] = optim.Adam(
                params=self.nets["log_qac_weight"].parameters(),
    ...
    def _train_policy_on_batch(self, batch, epoch, validate=False):
        ...
        pred_qs = torch.cat(pred_qs, dim=1).mean(dim=1, keepdim=True)
    ...
    def _train_critic_on_batch(self, batch, epoch, validate=False):
        ...
        with torch.no_grad():
            ...
            target_qs= torch.cat(target_qs, dim=1).mean(dim=1, keepdim=True)
        ...
        # Calculate QAC stuff
        qac_random_actions = torch.FloatTensor(N, B, A).uniform_(-1., 1.).to(self.device)                           # shape (N, B, A)
        qac_random_log_prob = np.log(0.5 ** A)
        qac_curr_actions, qac_curr_log_prob = self._get_actions_and_log_prob(dist=curr_dist, sample_shape=(N,))     # shape (N, B, A) and (N, B, 1)
        qac_next_actions, qac_next_log_prob = self._get_actions_and_log_prob(dist=next_dist, sample_shape=(N,))     # shape (N, B, A) and (N, B, 1)
        qac_curr_log_prob = qac_curr_log_prob.squeeze(dim=-1).permute(1, 0).detach()                                # shape (B, N)
        qac_next_log_prob = qac_next_log_prob.squeeze(dim=-1).permute(1, 0).detach()                                # shape (B, N)
        ...
        for critic, q_pred in zip(self.nets["critic"], q_preds):
            ...
            q_rand = self._get_qs_from_actions(obs_dict=batch["obs"], actions=qac_random_actions.permute(1, 0, 2), goal_dict=batch["goal_obs"], q_net=critic)
            q_curr = self._get_qs_from_actions(obs_dict=batch["obs"], actions=qac_curr_actions.permute(1, 0, 2), goal_dict=batch["goal_obs"], q_net=critic)
            q_next = self._get_qs_from_actions(obs_dict=batch["obs"], actions=qac_next_actions.permute(1, 0, 2), goal_dict=batch["goal_obs"], q_net=critic)
            q_cat = torch.cat([
                q_rand - qac_random_log_prob,
                q_next - qac_next_log_prob,
                q_curr - qac_curr_log_prob,
            ], dim=1)           # shape (B, 3 * N)
            q_cats.append(q_cat)
        ...
        qac_losses = []
        critic_losses = []
        qac_weight = torch.clamp(self.log_qac_weight.exp(), min=0.0, max=1000000.0)
        info["critic/qac_weight"] = qac_weight.item()
        for i, (q_pred, q_cat) in enumerate(zip(q_preds, q_cats)):
            # Calculate td error loss
            td_loss = self.td_loss_fcn(q_pred, q_target)
            # Calculate cql loss
            qac_loss = qac_weight * (self.min_q_weight * (torch.logsumexp(q_cat, dim=1).mean() - q_pred.mean()) -
                                     self.target_q_gap)
            qac_losses.append(qac_loss)
            # Calculate total loss
            loss = td_loss + qac_loss
        ...
        if not validate:
            ...
            if self.automatic_qac_tuning:
                qac_weight_loss = -torch.stack(qac_losses).mean()
                info[
                    "critic/qac_weight_loss"] = qac_weight_loss.item()  # Make sure to not store computation graph since we retain graph after backward() call
                self.optimizers["qac"].zero_grad()
                qac_weight_loss.backward(retain_graph=True)
                self.optimizers["qac"].step()
                info["critic/qac_grad_norms"] = self.log_qac_weight.grad.data.norm(2).pow(2).item()
    ...
    def _log_critic_info(self, info):
        ...
        if "critic/qac_weight_loss" in info:
            loss_log["Critic/QAC_Weight"] = info["critic/qac_weight"]
            loss_log["Critic/QAC_Weight_Loss"] = info["critic/qac_weight_loss"]
            loss_log["Critic/QAC_Grad_Norms"] = info["critic/qac_grad_norms"]
        return loss_log
