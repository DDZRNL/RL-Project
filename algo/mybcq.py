...
@register_algo_factory_func("mybcq")
def algo_config_to_class(algo_config):
    if algo_config.critic.distributional.enabled:
        return MYBCQ_Distributional, {}
...
class MYBCQ(PolicyAlgo, ValueAlgo):
    ...
    def _create_actor(self):
        ...
        actor_class = ValueNets.ActionValueNetwork
        actor_args = dict(
            obs_shapes=self.obs_shapes,
            ac_dim=self.ac_dim,
            mlp_layer_dims=self.algo_config.critic.layer_dims,
        ...
        self.nets["actor"] = nn.ModuleList()
        self.nets["actor_target"] = nn.ModuleList()
        for _ in range(self.algo_config.critic.ensemble.n):
            critic = actor_class(**actor_args)
            self.nets["actor"].append(critic)

            actor_target = actor_class(**actor_args)
            self.nets["actor_target"].append(actor_target)
    ...
    def _train_actor_on_batch(self, batch, action_sampler_outputs, critic_outputs, epoch, no_backprop=False):
        ...
        info["actor/q_targets"] = q_targets

        # Train all critics using this set of targets for regression
        actor_outputs = []
        for actor_ind, actor in enumerate(self.nets["actor"]):
            actor_loss, actor_output = self._compute_critic_loss(
                critic=actor, 
                states=s_batch, 
                actions=a_batch, 
                goal_states=goal_s_batch, 
            ...
            info["actor/actor{}_loss".format(actor_ind + 1)] = actor_loss
            actor_outputs.append(actor_output)

            if not no_backprop:
                actor_grad_norms = TorchUtils.backprop_for_loss(
                    net=self.nets["actor"][actor_ind],
                    optim=self.optimizers["critic"][actor_ind],
                    loss=actor_loss, 
                    max_grad_norm=self.algo_config.critic.max_gradient_norm,
                )
                info["actor/actor{}_grad_norms".format(actor_ind + 1)] = actor_grad_norms

        return info, actor_outputs
...
class MYBCQ_Distributional(MYBCQ):
    def _create_critics(self):
